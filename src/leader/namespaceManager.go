...
func (nm *namespaceManager) tree2array(array *[]serialTreeNode, node *nameSpaceTree) int {
...
// Serialize serializing the metadata for storing to disk
...
func (nm *namespaceManager) array2tree(array []serialTreeNode, id int) *nameSpaceTree {
	n := &nameSpaceTree{
	...
	if array[id].IsDir {
		n.children = make(map[string]*nameSpaceTree)
...
// Deserialize Deserializing the metadata from disk
...
func newNamespaceManager() *namespaceManager {
	nm := &namespaceManager{
		root: &nameSpaceTree{isDir: true,
			children: make(map[string]*nameSpaceTree)},
	}
	log.Info("----------------------")
...
func (nm *namespaceManager) lockParents(p gfs.Path, goDown bool) ([]string, *nameSpaceTree, error) {
...
func (nm *namespaceManager) Create(p gfs.Path) error {
	...
	cwd.children[filename] = new(nameSpaceTree)
	return nil
}
...
func (nm *namespaceManager) Mkdir(p gfs.Path) error {
	...
	cwd.children[filename] = &nameSpaceTree{isDir: true,
		children: make(map[string]*nameSpaceTree)}
	return nil
}
